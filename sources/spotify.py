...
TITLE_SANITISE = re.compile(r'[(\[][^)\]]*(?:lyrics|official|music).*[)\]] ?', re.IGNORECASE)
...
class SpotifyAudioTrack(DeferredAudioTrack):
    ...
    def _prioritise(self, external: AudioTrack) -> int:
        ...
        if 'remix' in lowered:
            ...
            if any(word in lowered for word in ('audio', 'radio')):
                ...
                score += 2  # Lyrics might be sped up due to people trying to evade CID.

        if external.author.endswith('- Topic'):
            score += 3

        duration_distance = max(self.duration, external.duration) - min(self.duration, external.duration)
    ...
    async def load(self, client):
        ...
        cached = r.get(set_key)

        if cached:
            return cached.decode()

        if not self.extra['isrc']:
            src = next(source for source in client.sources if isinstance(source, SpotifySource))
...
class SpotifySource(Source):
    ...
    async def _load_paginated(self, endpoint, offset: int = 0, **kwargs):
        ...
        tracks = SpotifyAudioTrack.from_items(map(lambda item: item.get('track', item), body['items']), **kwargs)
